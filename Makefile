...
ifeq ($(shell whoami),root)
	...
	$(LUAROCKS) install rockspec/princess-main-0.rockspec --tree=deps --only-deps --local $(LUAROCKS_SERVER_OPT)
...
install:
	$(INSTALL) -d $(INST_LUADIR)/man
	$(INSTALL) man/*.lua $(INST_LUADIR)/man/

	$(INSTALL) -d $(INST_LUADIR)/man/cli
	$(INSTALL) man/cli/*.lua $(INST_LUADIR)/man/cli/

	$(INSTALL) -d $(INST_LUADIR)/man/stream
	$(INSTALL) man/stream/*.lua $(INST_LUADIR)/man/stream/
	
#publish:
#	luarocks upload rockspec/princess-0.0.1-0.rockspec --skip-pack --api-key=