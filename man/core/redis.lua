...
function _M.get(config, key)
    return _M.exec(config, r_get, key)
end
...
function _M.set(config, key, value, ttl)
    return _M.exec(config, r_set, key, value, ttl)
end
...
function _M.expire(config, key, ttl)
    return _M.exec(config, r_expire, key, ttl)
end
...
function _M.ttl(config, key)
    return _M.exec(config, r_ttl, key)
end
...
function _M.publish(config, key, value)
    return _M.exec(config, r_publish, key, value)
end
...
function _M.script(config, cmd, value)
    return _M.exec(config, r_script, cmd, value)
end
...
function _M.evalsha(config, ...)
    return _M.exec(config, r_evalsha, ...)
end
